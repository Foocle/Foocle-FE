...
const TitleSection = styled.div`
  ...
  border-radius: 15px;
  background-image: linear-gradient(#e7e7e7, #e7e7e7), linear-gradient(170deg, #e7e7e7, #a2a2a2);
...
const VideoComplete = () => {
  ...
  const {
    videoUrl = '', // 기본값으로 빈 문자열 또는 예시 비디오 URL
    promotionText = 'AI가 추천하는 홍보 문구입니다.', // 기본값
    suggestedTitle = 'AI 추천 제목',
    tags: incomingTags = ['#추천태그'],
  } = state || {};
  ...
  return (
    <PageWrapper>
      ...
      <TextSection>
        ...
        <TextArea value={script} maxLength={SCRIPT_MAX} onChange={(e) => setScript(e.target.value)} minRows={1} />
      </TextSection>
      ...
      <TextSection>
        ...
        <TagWrap>
          {tags.map((t) => (
            <Tag key={t} $active={selected.has(t)} onClick={() => toggleTags(t)} aria-pressed={selected.has(t)}>
              {t}
            </Tag>
          ))}
        </TagWrap>
